...
using SpecializationService.Controllers;
...
builder.Services.AddDbContext<AppDbContext>(opt => opt.UseInMemoryDatabase("Specialization"));
builder.Services.AddScoped<ISpecializationRepo, SpecializationRepo>();
builder.Services.AddAutoMapper(AppDomain.CurrentDomain.GetAssemblies());
builder.Services.AddHttpClient<SpecializationController>();
...
if (app.Environment.IsDevelopment())
{
    ...
    app.UseSwaggerUI(c => 
    {
        c.SwaggerEndpoint("/swagger/v1/swagger.json", "SpecializationService v1");
    });
}
